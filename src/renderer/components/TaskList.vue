...
export default {
  ...
  computed: {
    ...
    projectTasks: function () {
      ...
      const deletedProjects = this.$store.getters.getDeletedProjects
      ...
      const projectsToConsider = this.$store.getters.getStoredDescendantProjectIdsOfSelected
      const allTasksToShow = ts.filter(task => projectsToConsider.includes(task.project))

      const mapOfTasksPerWhen = { today: [], thisweek: [], waitingfor: [], someday: [] }
    ...
    numberOfCompletedProjectTasks: function () {
      if (this.projectTasks) {
        const completedTasksToday = this.projectTasks.today.filter(t => t.completed)
        const completedTasksThisWeek = this.projectTasks.thisweek.filter(t => t.completed)
        const completedTasksWaitingFor = this.projectTasks.waitingfor.filter(t => t.completed)
        const completedTasksSomeday = this.projectTasks.someday.filter(t => t.completed)
    ...
    numberOfProjectTasks: function () {
      if (this.projectTasks) {
        const completedTasksToday = this.projectTasks.today
        const completedTasksThisWeek = this.projectTasks.thisweek
        const completedTasksWaitingFor = this.projectTasks.waitingfor
        const completedTasksSomeday = this.projectTasks.someday
        return completedTasksToday.length + completedTasksThisWeek.length + completedTasksSomeday.length + completedTasksWaitingFor.length
      } else {
        return 0
      }
  ...
  methods: {
    ...
    addTask: function () {
      const taskName = this.newTaskText && this.newTaskText.trim()
      if (!taskName) {
        return
      }
    ...
    moveUpTask: function (task) {
      ...
      const taskUpToSwap = this.findPreviousVisible(task)
      if (!taskUpToSwap) {
        return
      }

      const newOrder = taskUpToSwap.order

      if (task.when === 'thisweek') {
        const lastIndexToday = this.findLastIndexInToday()
      ...
      if (task.when === 'waitingfor') {
        const lastIndexThisWeek = this.findLastIndexInThisWeek()
      ...
      if (task.when === 'someday') {
        const lastIndexThisWaitingFor = this.findLastIndexInWaitingFor()
    ...
    selectPreviousTask: function (task) {
      const previousTask = this.findPreviousVisible(task)
      if (!previousTask) {
        return
      }
    ...
    selectNextTask: function (task) {
      const nextTask = this.findNextVisible(task)
      if (!nextTask) {
        return
      }
    ...
    moveDownTask: function (task) {
      const taskDownToSwap = this.findNextVisible(task)

      if (!taskDownToSwap) {
        return
      }

      const newOrder = taskDownToSwap.order
      ...
      if (task.when === 'someday') {
        const lastIndexSomeday = this.findLastIndexInSomeday()
    ...
    findFirstVisible: function () {
      const taskList = this.projectTasksFlat
    ...
    findNextVisible: function (task) {
      let nextTask
      const nextTasks = this.projectTasksFlat.filter(t => t.order > task.order)
    ...
    removeCompleted: function () {
      const projectsToConsiderForRemoval = this.$store.getters.getStoredDescendantProjectIdsOfSelected
      const newTasks = []
      const ordersOfRemovedTasks = []
      this.tasks.forEach(task => {
        const keepTask = !projectsToConsiderForRemoval.includes(task.project) || !task.completed
        if (keepTask) {
          newTasks.push(task)
        } else {
    ...
    duplicateTask: function (task) {
      ...
      const lastIndex = this.findLastIndex(task.when)
    ...
    setWhen: function (task, when) {
      ...
      const newIndex = this.findLastIndex(when) + 1

...
export default {
  ...
  created () {
    ...
    this.$projectDb.find({}, function (err, docs) {
      ...
      if (!docs || docs.length === 0) {
        ...
        const allProjects = { name: 'All Projects', id: 2 }
  ...
  methods: {
    ...
    removeFromRoot: function (uuid) {
      const removeFromChildren = function (project, id) {
        if (!project.children) {
          return
        }
        const childrenIds = project.children.map(p => p.id)
        if (childrenIds.includes(id)) {
          const filtered = project.children.filter(p => p.id !== uuid)

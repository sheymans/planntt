...
preferences.find({}, function (err, docs) {
  ...
  if (!docs || docs.length === 0) {
    ...
    console.log('no preferences specified using data location for tasks: ' + dataLocation)
  } else {
    let dataLocationObject = docs[0]
    dataLocation = dataLocationObject['dataLocation']
    console.log('using data location for tasks: ' + dataLocation)
  }
})

export default new Datastore({
  autoload: true,
  filename: path.join(dataLocation, '/tasks.db')
})
